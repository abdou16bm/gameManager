...
//let data_update = input
    ...
    form.parse(req, function (err, fields, files) {
      ...
      if (err) console.log('error',err);
        doc_module.uploadFile('./../docDyn/public/img/user/',id,files,'main','main','jpg', function (err,count1){

        if (err) console.log(err)

        console.log('count1 : ',count1)
         doc_module.uploadMultiFile('./../docDyn/public/img/user/',id,files,'jpg', function (err,count2){
...
const user_edit_save2 = function (req,res) {
     const id = req.params.id;
 //let input = JSON.parse(JSON.stringify(req.body));
 //let data_update = input
     const options = {
     ...
     form.parse(req, function (err, fields, files) {
         ...
         data_update.user_id = input.user_id;
         data_update.user_username = input.signup_username;
 
         data_update.user_name = input.user_name;
         data_update.user_lastname = input.user_lastname;
         data_update.priv_id = 3;
 
         user_module.user_update(input.user_id,data_update,function (err,result1) {
             if (err) console.log('error',err);
 
             data_update.user_password = input.signup_password;
             user_module.user_update_password(input.user_id,data_update,function (err,result2) {
                 if (err) console.log('error',err);
 ...
 exports.user_edit_save2 = user_edit_save2
...
if (err) console.log('error',err);
           doc_module.uploadFile('./../docDyn/public/img/user/',result1.insertId,files,'main','main','jpg', function (err,count1){

           if (err) console.log(err)

            console.log('count1 : ',count1)
                doc_module.uploadMultiFile('./../docDyn/public/img/user/',result1.insertId,files,'jpg', function (err,count2){
